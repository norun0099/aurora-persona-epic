...
import json
from aurora_memory.utils.git_helper import push_file_to_github
...
def try_auto_save(memory_text: str, prefix: str = "constitution"):
    ...
    push_result = push_file_to_github(file_path, f"Auto-save {prefix} at {now}")
...
def save_memory_record(data: dict):
    ...
    push_result = push_file_to_github(file_path, f"Add new memory {file_path.name}")
